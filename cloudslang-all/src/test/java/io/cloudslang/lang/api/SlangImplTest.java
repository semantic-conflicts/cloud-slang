...
/** 
 * User: stoneo Date: 04/12/2014 Time: 11:00
 */
@RunWith(SpringJUnit4ClassRunner.class) @ContextConfiguration(classes=SlangImplTest.Config.class) public class SlangImplTest {
  public static CompilationArtifact emptyCompilationArtifact=new CompilationArtifact(new ExecutionPlan(),new HashMap<String,ExecutionPlan>(),new ArrayList<Input>(),new ArrayList<Input>());
  public static int ALL_EVENTS_SIZE=23;
  @Autowired public Slang slang;
  @Autowired public SlangCompiler compiler;
  @Autowired public Score score;
  @Autowired public EventBus eventBus;
  ...
  @Test(expected=IllegalArgumentException.class) public void testCompileWithNoFilePath(){
    slang.compile(null,new HashSet<SlangSource>());
  }
  @Test(expected=IllegalArgumentException.class) public void testCompileWithCorruptedFilePath(){
    slang.compile(SlangSource.fromFile(new File("/")),new HashSet<SlangSource>());
  }
  @Test public void testCompileWithNullDependencies() throws IOException {
    SlangSource tempFile=createTempFile();
    Mockito.when(compiler.compile(any(SlangSource.class),anySetOf(SlangSource.class))).thenReturn(emptyCompilationArtifact);
  ...
  @Test public void testCompileWithDependencies() throws IOException {
    SlangSource tempFile=createTempFile();
  ...
  @Test(expected=Exception.class) public void testCompileOperationWithException() throws IOException {
  ...
  @Test public void testRunWithNullInputs(){
    Long executionId=slang.run(emptyCompilationArtifact,null,null);
    Assert.assertNotNull(executionId);
  }
  @Test(expected=IllegalArgumentException.class) public void testRunWithNullCompilationArtifact(){
    slang.run(null,new HashMap<String,Serializable>(),null);
  }
  @Test public void testLaunchOperation() throws IOException {
  ...
  @Test public void testLaunch() throws IOException {
    Slang mockSlang=Mockito.mock(SlangImpl.class);
    SlangSource tempFile=createTempFile();
    Mockito.when(mockSlang.compileAndRun(any(SlangSource.class),anySetOf(SlangSource.class),anyMapOf(String.class,Serializable.class),anyMapOf(String.class,Serializable.class))).thenCallRealMethod();
    Long id=mockSlang.compileAndRun(tempFile,new HashSet<SlangSource>(),new HashMap<String,Serializable>(),new HashMap<String,Serializable>());
  ...
  @Test public void testSubscribeOnEventsWithListener(){
  ...
  @Test public void testUnSubscribeOnEvents(){
    ScoreEventListener eventListener=new EventListener();
    slang.unSubscribeOnEvents(eventListener);
    Mockito.verify(eventBus).unsubscribe(eventListener);
  }
  @Test public void testSubscribeOnAllEventsWithListener(){
    Slang mockSlang=Mockito.mock(SlangImpl.class);
    ScoreEventListener eventListener=new EventListener();
    ...
    Mockito.verify(mockSlang).subscribeOnEvents(eq(eventListener),argThat(new Predicate<Set<String>>(){
      @Override public boolean apply(      Set<String> item){
        return item.size() == ALL_EVENTS_SIZE;
      }
    }
));
  }
public class EventListener implements ScoreEventListener {
    @Override public void onEvent(    ScoreEvent event) throws InterruptedException {
    }
    public EventListener(){
    }
  }
  public SlangSource createTempFile() throws IOException {
    File tempFile=File.createTempFile("temp",null);
    tempFile.deleteOnExit();
    return SlangSource.fromFile(tempFile);
  }
@Configuration static class Config {
    @Bean public SlangImpl slang(){
      return new SlangImpl();
    }
    @Bean public SlangCompiler compiler(){
      SlangCompiler compiler=Mockito.mock(SlangCompiler.class);
      Mockito.when(compiler.compile(any(SlangSource.class),anySetOf(SlangSource.class))).thenReturn(emptyCompilationArtifact);
      return compiler;
    }
    @Bean public Score score(){
